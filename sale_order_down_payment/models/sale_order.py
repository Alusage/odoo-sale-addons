...
class SaleOrder(models.Model):
    ...
    down_payment_amount = fields.Monetary(string="Down payment amount", compute="get_down_payment_amount")
    display_dpa_text = fields.Boolean(string="Display down payment amount in letters on report", default=True)
    down_payment_amount_text = fields.Char(string="Down payment amount in letters", compute="get_dpa_text")
    ...
    def get_dpa_text(self):
        ...
        for order in self:
            ...
            if odpa:
                order.down_payment_amount_text = order.currency_id.amount_to_text(odpa)
